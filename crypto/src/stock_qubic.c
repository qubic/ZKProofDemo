...
// Origin: verbatim copy of Qubic core src/kangaroo_twelve.h + src/four_q.h (https://github.com/qubic/core),
// Anti-Military License (crypto/LICENSE-QUBIC.md). KangarooTwelve derives from XKCP (CC0 / public domain);
// FourQ derives from MSR FourQlib (MIT, crypto/LICENSE-FourQlib.md). Reference implementation, host-only,
// used as the test oracle for riscv_fourq_verify.c. Treat as read-only. See NOTICE.
...
static void multiply(const unsigned long long* a, const unsigned long long* b, unsigned long long* c)
{
    ...
    // Temporary forces _umul128 to finish before the addition (compiler evaluation-order hazard).
...
static bool verify_fourq_signature(const unsigned char* publicKey, const unsigned char* signature, const unsigned char* data, const unsigned long long dataLen)
{
    unsigned char messageDigest[32] = {0};
    KangarooTwelve(data, dataLen, messageDigest, 32);
